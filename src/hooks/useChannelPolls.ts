...
export const useChannelPolls = (conversationId?: string) => {
  ...
  const getPoll = useCallback(async (messageId: string): Promise<PollData | null> => {
    try {
  ...
  const vote = useCallback(async (pollId: string, optionIndex: number): Promise<boolean> => {
    try {
  ...
  return { createPoll, getPoll, vote, loading };
};
